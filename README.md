...
reproduction number generated by `dynamic_beta_ML`. Note that if
`savetofile = true` the script does not consider any files already
stored with the same name, i.e., overwriting of files are possible.
```
reg=2; % Uppsala
...
`weekly_prediction`, see example Fig. 3 for usage.
...
samples are generated using a national weighted mean posterior. See
example Fig. S8 below for code sample. The following example will
generate the bootstrap replicate samples, infer one posterior using
KLAM on that data for the Uppsala region, and finally perform the
upscaling of the reproduction number on that posterior. *NOTE* that it
is time consuming.
```
savetofile = true;
