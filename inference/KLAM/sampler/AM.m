...
for i = start:stop
  ...
  if any(strcmp(param.dynamic,'k_sew'))

    % if waste included, replicate same k_sew for rep_slabs
    % consecutive slabs
    
    nslab = param.nslab; % number of slabs 
    rep_slabs = int16(3); % keep k_sew constant over 3 slabs
    n_values = idivide(nslab,rep_slabs); % number of unique values of k_sew needed
    
    % find indices for where k_sew belong, then replicate the
    % values accordingly
    
    flags = contains(param.ratenames,'k_sew');
    flags_ind = find(flags);
    
    first_k_sew = flags_ind(1);
    last_k_sew = flags_ind(end);
    
    k = 0;
    for j = first_k_sew:last_k_sew
      eps(j) = eps(first_k_sew+k*rep_slabs);
      if mod(j-first_k_sew+1,rep_slabs) == 0  && k < n_values-1 
        k = k+1;
      end
    end
  end
