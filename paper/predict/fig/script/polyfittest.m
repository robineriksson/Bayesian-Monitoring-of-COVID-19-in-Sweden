...
fitalpha68 = 0.32; % 68%
register = 'RU'; % data source
reg = 'Uppsala'; % data region
startdate = 200401;
enddate = 210531;
kalmancompare = 0;%load in full kalman data or not.
...
%% RMSE for the linear filter
TAB95 = cell(3,3);
TAB68 = cell(3,3);

% error function
RMSE = @(x,y) sqrt(sum( (x-y).^2,2));
...
% Compared to RMSE on Kalman on the selected predictions
...
rmse_kalman_reported = [RMSE(kalman_data.H, kalman_pred.H),...
  RMSE(kalman_data.W, kalman_pred.W),...
  RMSE(kalman_data.D, kalman_pred.D)];


if kalmancompare
  lagplot % to get muhat and Ydata_swe
  %%
  rmse_kalman_all = sum(RMSE(exp(muhat(:,1:end-7)),Ydata_swe));
else
  rmse_kalman_all = NaN;
end
...
legend(leg, ...
  ...
  rmse_lm_all = [RMSE(H, val.H(1:end-7,end)')
                 RMSE(W, val.W(1:end-7,end)')
                 RMSE(D, val.D(1:end-7,end)')];
  
 
  % find the matching dates
  data_sel = find(ismember(data.date(:,end),date_comp));
  pred_sel = [data_sel(1)-7;  data_sel(1:end-1)];

  %sum(data.x(data_sel,end) - val.x(pred_sel,end)) % checks the date match
  rmse_lm_sel = [RMSE(data.H(data_sel,end)', val.H(pred_sel,end)')
                 RMSE(data.W(data_sel,end)', val.W(pred_sel,end)')
                 RMSE(data.D(data_sel,end)', val.D(pred_sel,end)')];
               
  %TAB(useweights+2,:) = [{sum(rmse_lm_sel)} {sum(rmse_lm_all)}];
...
%   inside_all = 1-mean(elem_all,2);
  ...
  elem_sel95 = [val.Hci95(pred_sel,end,1)' > data.H(data_sel,end)' | val.Hci95(pred_sel,end,2)' < data.H(data_sel,end)'
              val.Wci95(pred_sel,end,1)' > data.W(data_sel,end)' | val.Wci95(pred_sel,end,2)' < data.W(data_sel,end)'
              val.Dci95(pred_sel,end,1)' > data.D(data_sel,end)' | val.Dci95(pred_sel,end,2)' < data.D(data_sel,end)'];
  inside_sel95 = 1-mean(elem_sel95,2);
  
  elem_sel68 = [val.Hci68(pred_sel,end,1)' > data.H(data_sel,end)' | val.Hci68(pred_sel,end,2)' < data.H(data_sel,end)'
              val.Wci68(pred_sel,end,1)' > data.W(data_sel,end)' | val.Wci68(pred_sel,end,2)' < data.W(data_sel,end)'
              val.Dci68(pred_sel,end,1)' > data.D(data_sel,end)' | val.Dci68(pred_sel,end,2)' < data.D(data_sel,end)'];
  inside_sel68 = 1-mean(elem_sel68,2);
  
  TAB95(useweights+2,:) = num2cell(inside_sel95)';
  TAB68(useweights+2,:) = num2cell(inside_sel68)';
...
TAB68 = cat(1,[{'H'} {'W'} {'D'}],TAB68);
TAB68 = cat(2,[{' '} {'Kalman filter (68\%%)'} {'Simple linear'} {'Weighted linear'}]',TAB68);
TAB95 = cat(2,[{'Kalman filter (95\%%)'} {'Simple linear'} {'Weighted linear'}]',TAB95);
...
caption = ['Frequency (\%%) of all weekly reported predictions for Uppsala' ...
           ...
           ' the following week (N~=~' num2str(numel(pred_sel)) ') using the comparative models.'];
